...
That makes Path 6, verifiable reasoning traces, the legibility strategy rather than roles. The trace is inspectable because it was never summarized away. The remaining work is tooling: visualizing a long interleaved deliberation so an auditor can follow it, which is unsolved and is where tooling companies should be. It is also a direct answer to a question the research has posed but not settled — [PatchBoard (2026)](https://arxiv.org/abs/2605.29313) notes that the field has established shared state matters while leaving open how that state should be updated, authorized, and audited over long horizons. Update through the board, authorize through capabilities, audit through the rendered history. Read this layer as a candidate answer to that question, and judge it as one.
